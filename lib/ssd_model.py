    ...
    def build(self):
        with tf.device(self.device):
            with tf.name_scope("block1"):
                x = tf.layers.Conv2D(64, (3, 3), padding="same", name="conv1_1")(self.inputs)
                x = tf.layers.BatchNormalization(momentum=0.1,)(x)
                x = tf.nn.relu(x)
                x = tf.layers.Conv2D(64, (3, 3), padding="same", name="conv1_2")(x)
                x = tf.layers.BatchNormalization(momentum=0.1,)(x)
                x = tf.nn.relu(x)
                # 150*150*64
        ...
        x = tf.layers.Conv2D(1024, (3, 3), dilation_rate=6, padding="same",  # 关于扩张卷积
                             name="conv6")(x)  # 19*19*512
        x = tf.layers.BatchNormalization(momentum=0.1)(x)
        x = tf.nn.relu(x)

        x = tf.layers.Conv2D(1024, (1, 1), padding="same", name="conv7")(x)
        x = tf.layers.BatchNormalization(momentum=0.1)(x)
        x = tf.nn.relu(x)
        ...
        x = tf.layers.Conv2D(512, (3, 3), padding="valid", strides=(2, 2), name="conv8_2")(x)  # 10*10*512
        x = tf.layers.BatchNormalization(momentum=0.1)(x)
        x = tf.nn.relu(x)
        ...
        x = tf.layers.Conv2D(256, (3, 3), padding="valid", strides=(2, 2), name="conv9_2")(
            x)  # 5*5*256
        x = tf.layers.BatchNormalization(momentum=0.1)(x)
        x = tf.nn.relu(x)
        ...
        x = tf.layers.Conv2D(256, (3, 3), padding="valid", name="conv10_2")(x)  # 3*3*256
        x = tf.layers.BatchNormalization(momentum=0.1)(x)
        x = tf.nn.relu(x)
        self.end_point['conv10'] = x
        x = tf.layers.Conv2D(128, (1, 1), activation="relu", name="conv10_1")(x)
        x = tf.layers.Conv2D(256, (3, 3), name="conv10_2")(x)  # 1*1*256
        x = tf.layers.BatchNormalization(momentum=0.1)(x)
        x = tf.nn.relu(x)
        ...
        self.cls_preds = tf.reshape(self.cls_preds, [self.batch_size,
                                                     -1,
                                                     self.n_class])
        return self.loc_preds, self.cls_preds  # (b,-1,4),(b,-1,20)
